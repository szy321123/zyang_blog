...
> Git 是目前最流行的版本控制系统，GitHub 则是全球最大的代码托管平台。两者结合可以让团队协作高效且有条不紊。
...
理解 Git 的核心概念是高效使用它的前提。这些概念会贯穿你后续所有的 Git 操作。
...
Git 管理下的仓库有**三个主要区域**，理解它们之间的流转关系至关重要：
...
**流程说明**：工作目录 → 暂存区 → 本地仓库 → 远程仓库，是代码提交的标准路径。
...
正确的配置能让你在后续使用时少走弯路，特别是在与 GitHub 交互时。
...
{% note info %}
**Windows** 用户前往 [git-scm.com](https://git-scm.com) 下载 Git Bash 安装即可。
{% endnote %}
...
与 GitHub 通信推荐使用 SSH 密钥，可以省去每次输入密码的麻烦。
...
{% note info %}
如果你的 GitHub 账号较老，不支持 Ed25519，可以用 RSA：
```bash
ssh-keygen -t rsa -b 4096 -C "your_email@example.com"
```
**Ed25519** 性能更好、安全性更高，优先选择。
{% endnote %}
...
熟练掌握这些基础命令是日常开发的第一步。它们是你与 Git 交互的最基本工具。
...
查看工作目录状态是你每次开始工作前首先要做的。

**查看工作目录状态：**

```bash
git status                      # 查看工作目录状态（最常用）
git status -s                   # 简洁模式（M = 已修改，A = 已暂存，?? = 未跟踪）
git status --short              # 同上
```
...
{% note warning %}
`git add .` 会暂存当前目录所有修改，初学者慎用，容易不小心提交不该提交的文件（如编译产物、本地配置）。
{% endnote %}
...
{% note warning %}
**`git reset --hard` 是危险操作**，会丢失未提交的修改。执行前务必确认你已经提交了所有重要工作。`git reflog` 可以恢复误删的提交。
{% endnote %}

### 3.5 恢复误删的提交

Git 的操作几乎都是可逆的，即使你做了危险的 reset 也可以恢复：
...
分支是 Git 最强大的功能之一，让多人并行开发和功能隔离变得轻而易举。
...
当你在某个分支上有未提交的修改，但需要紧急切换到其他分支处理问题时，用 stash 可以临时保存工作状态：
...
标签用于标记特定的提交点，通常用于发布版本，是版本管理的基础。
...
了解标准的 GitHub 协作流程是与开源项目或团队高效合作的前提。
...
Pull Request（PR）是 GitHub 协作的核心，理解它的工作机制可以让协作更顺畅。
...
Code Review 是保证代码质量和团队知识共享的关键环节，越来越被重视。
...
冲突是多人协作中不可避免的情况，掌握处理方法是团队协作的必备技能。
...
**说明**：main 用于生产环境，develop 是开发主线，release 分支用于发布准备，hotfix 用于紧急修复，feature 用于功能开发。
...
这些高级技巧可以帮你解决特殊场景下的问题，提升 Git 使用效率。
